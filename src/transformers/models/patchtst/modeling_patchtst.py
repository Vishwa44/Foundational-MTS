...
class PatchTSTBatchNormChannelAttn(nn.Module):
    """
    Compute batch normalization over the sequence length (time) dimension.
    """

    def __init__(self, config: PatchTSTConfig, num_patches):
        super().__init__()
        self.batchnorm = nn.BatchNorm1d(config.d_model*num_patches, eps=config.norm_eps)
...
class PatchTSTEncoderLayer(nn.Module):
    ...
    def __init__(self, config: PatchTSTConfig):
        ...
        self.new_channel_attention = config.new_channel_attention
        ...
        self.num_patches = (max(config.context_length, config.patch_length) - config.patch_length) // config.patch_stride + 1
    ...
    def forward(self, hidden_state: torch.Tensor, output_attentions: Optional[bool] = None):
        ...
        if self.channel_attention:
            ...
            if self.new_channel_attention:
                ...
                # print("in old channel attention")
